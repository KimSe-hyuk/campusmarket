...
<div style="display: none;" id="userDiv">
	...
	$sale_status = empty($_REQUEST["sale_status"]) ? "0" : $_REQUEST["sale_status"];
?>
...
<div>
			...
			<h1>나의 판매목록</h1>
				...
				<div style="display: none;  padding: 15px; background: #007FFF; box-shadow: 0px 3px 3px rgb(0 0 0 / 25%); border-radius: 16px;text-align: right;" id="myDIV">
                    <ul>
                        <li><a style="color: #FFFFFF;" href="mysell.php?category=<?=$category?>&share=3 &sale_status=0"><img src="/campusmarket/image/filteron.png" style="padding-right:11px;"/>판매중</a>    </li>      
                        <li style="margin-top: 13px;padding-bottom: 12px;"><a style="color: #FFFFFF;" href="mysell.php?category=<?=$category?>&share=3&sale_status=1">판매완료</a>  </li>     
                        <li style="padding-bottom: 12px;"><a style="color: #FFFFFF;" href="mysell.php?category=<?=$category?>&share=4&sale_status=0">나눔중</a>  </li>
                        <li ><a style="color: #FFFFFF;" href="mysell.php?category=<?=$category?>&share=4&sale_status=1">나눔완료</a>    </li>         
                    </ul>
				</div>
    ...
    $query = $db->query("select * from board_$bid where member_num=$userNum and (share='판매' or share='나눔') and object_category like '%$category%' and share like '%$share%' and sale_status like '%$sale_status%' order by object_num desc limit $start, $numLines");
